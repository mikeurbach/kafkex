defmodule Kafkex.Protocol.Offsets do
  ...
  defmodule Partition do
    ...
    def parse(<<partition::32, partition_error_code::16, offsets_length::32, rest::binary>>) do
      {offsets, new_rest} =
        parse_list(offsets_length, rest, fn <<offset::64, rest::binary>> -> {offset, rest} end)

      {%Kafkex.Protocol.Offsets.Partition{
         partition: partition,
  ...
  defmodule TopicPartitions do
    ...
    def parse(
      ...
      {partitions, new_rest} =
        parse_list(partitions_length, rest, &Kafkex.Protocol.Offsets.Partition.parse/1)
    ...
    def parse({:ok, <<correlation_id::32, topic_partitions_length::32, rest::binary>>}) do
      {topic_partitions, <<>>} =
        parse_list(
          ...
          rest,
          &Kafkex.Protocol.Offsets.TopicPartitions.parse/1
        )

      {correlation_id,
       {:ok, %Kafkex.Protocol.Offsets.Response{topic_partitions: topic_partitions}}}
    end
  end
end
