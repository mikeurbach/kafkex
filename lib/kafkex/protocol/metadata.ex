defmodule Kafkex.Protocol.Metadata do
  ...
  defmodule Request do
    ...
    def build(correlation_id, client_id, options \\ []) do
      case options |> Keyword.get(:topics) do
        ...
        topics when is_list(topics) -> build_headers(@api_key, @api_version, correlation_id, client_id) <> build_primitive_list(topics)
      end
    end
  end
  ...
  defmodule TopicMetadata do
    ...
    def build(<< topic_error_code :: 16, topic_length :: 16, topic :: size(topic_length)-binary, partitions_length :: 32, rest :: binary>>) do
      {partition_metadatas, rest} = parse_list(partitions_length, rest, &Kafkex.Protocol.Metadata.PartitionMetadata.build/1)
      {%TopicMetadata{error_code: error_code(topic_error_code), topic: topic, partition_metadatas: partition_metadatas}, rest}
    end
  end
  ...
  defmodule PartitionMetadata do
    ...
    def build(<< partition_error_code :: 16, partition_id :: 32, leader :: 32, replicas_length :: 32, rest :: binary>>) do
      {replicas, << isr_length :: 32, rest :: binary >>} = parse_list(replicas_length, rest, fn <<replica :: 32, rest :: binary>> -> {replica, rest} end)
      {isr, rest} = parse_list(isr_length, rest, fn <<isr :: 32, rest :: binary>> -> {isr, rest} end)
      {%PartitionMetadata{error_code: error_code(partition_error_code), partition_id: partition_id, leader: leader, replicas: replicas, isr: isr}, rest}
    end
  end
end
