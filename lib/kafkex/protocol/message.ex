defmodule Kafkex.Protocol.Message do
  ...
  def parse(
        bytes_remaining,
        <<offset::64, _size::32, _crc::32, 0::8, _attributes::8, key_size::32, rest::binary>>,
        items
      ) do
  ...
  def parse(
        bytes_remaining,
        <<offset::64, _size::32, _crc::32, 1::8, _attributes::8, ts::64, key_size::32-signed,
          rest::binary>>,
        items
      ) do
  ...
  def parse(bytes_remaining, rest, items) do
    ...
    Logger.warn(
      "[#{__MODULE__}][#{inspect(self())}] found #{bytes_remaining} bytes remaining! extra: #{
  ...
  defp parse_helper(base_size, bytes_remaining, offset, key_size, binary, items, ts \\ nil) do
    ...
    if value_size <= byte_size(rest) do
      ...
      parse(bytes_remaining - bytes_parsed, new_rest, [
