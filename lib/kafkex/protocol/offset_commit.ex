defmodule Kafkex.Protocol.OffsetCommit do
  ...
  defmodule Partition do
    ...
    def parse(<<partition::32, partition_error_code::16, rest::binary>>) do
      {%Kafkex.Protocol.OffsetCommit.Partition{
         partition: partition,
  ...
  defmodule TopicPartitions do
    ...
    def parse(
      ...
      {partitions, new_rest} =
        parse_list(partitions_length, rest, &Kafkex.Protocol.OffsetCommit.Partition.parse/1)

      {%Kafkex.Protocol.OffsetCommit.TopicPartitions{topic: topic, partitions: partitions},
       new_rest}
    end
  end
  ...
  defmodule Request do
    ...
    def build(correlation_id, client_id,
      ...
      topic_partitions_list =
        topic_partitions |> build_list(&Kafkex.Protocol.OffsetCommit.TopicPartitions.build/1)
    ...
    def parse({:ok, <<correlation_id::32, topic_partitions_length::32, rest::binary>>}) do
      {topic_partitions, <<>>} =
        parse_list(
          ...
          rest,
          &Kafkex.Protocol.OffsetCommit.TopicPartitions.parse/1
        )

      {correlation_id,
       {:ok, %Kafkex.Protocol.OffsetCommit.Response{topic_partitions: topic_partitions}}}
    end
  end
end
