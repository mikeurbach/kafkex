defmodule Kafkex.Protocol.SyncGroup do
  ...
  defmodule PartitionAssignment do
    ...
    def build(%PartitionAssignment{topic: topic, partitions: partitions}, rest) do
      rest <> build_item(topic) <> << length(partitions) :: 32 >> <> Enum.reduce(partitions, <<>>, fn(partition, others) -> others <> << partition :: 32 >> end)
    end
  ...
  defmodule MemberAssignment do
    ...
    def build(%MemberAssignment{version: version, partition_assignments: partition_assignments, data: data}) do
      build_item(<< version :: 16 >> <> << length(partition_assignments) :: 32 >> <> Enum.reduce(partition_assignments, <<>>, &PartitionAssignment.build/2) <> build_item(data, 32), 32)
    end

    def parse(<< version :: 16, partition_assignment_length :: 32, rest :: binary >>) do
      {partition_assignments, << _data_length :: 32, data :: binary >>} = parse_list(partition_assignment_length, rest, &Kafkex.Protocol.SyncGroup.PartitionAssignment.parse/1)
      %MemberAssignment{version: version, partition_assignments: partition_assignments, data: data}
    end
  end
  ...
  defmodule Request do
    ...
    def build(correlation_id, client_id, [group_id: group_id, generation_id: generation_id, member_id: member_id, group_assignment: group_assignment]) do
      build_headers(@api_key, @api_version, correlation_id, client_id) <> build_item(group_id) <> << generation_id :: 32 >> <> build_item(member_id) <> << length(group_assignment) :: 32 >> <> build_group_assignment(group_assignment)
    end

    defp build_group_assignment([]), do: <<>>
    defp build_group_assignment([{member_id, member_assignment} | rest]) do
      build_item(member_id) <> MemberAssignment.build(member_assignment) <> build_group_assignment(rest)
    end
  end
