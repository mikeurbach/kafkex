defmodule Kafkex.Protocol.OffsetFetch do
  ...
  defmodule TopicPartitions do
    ...
    def parse(<< topic_length :: 16, topic :: size(topic_length)-binary, partitions_length :: 32, rest :: binary >>) do
      {partitions, new_rest} = parse_list(partitions_length, rest, &Kafkex.Protocol.OffsetFetch.Partition.parse/1)
      {%Kafkex.Protocol.OffsetFetch.TopicPartitions{topic: topic, partitions: partitions}, new_rest}
    end
  end
  ...
  defmodule Request do
    ...
    def build(correlation_id, client_id, [group_id: group_id, topic_partitions: topic_partitions]) do
      topic_partitions_list = topic_partitions |> build_list(&Kafkex.Protocol.OffsetFetch.TopicPartitions.build/1)
      [build_headers(@api_key, @api_version, correlation_id, client_id), build_item(group_id), topic_partitions_list]
    end
  end
  ...
  defmodule Response do
    ...
    def parse({:ok, << correlation_id :: 32, topic_partitions_length :: 32, rest :: binary >>}) do
      {topic_partitions, <<>>} = parse_list(topic_partitions_length, rest, &Kafkex.Protocol.OffsetFetch.TopicPartitions.parse/1)
      {correlation_id, {:ok, %Kafkex.Protocol.OffsetFetch.Response{topic_partitions: topic_partitions}}}
    end
  end
end
