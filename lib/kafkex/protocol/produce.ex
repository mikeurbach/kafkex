defmodule Kafkex.Protocol.Produce do
  ...
  defmodule PartitionResponse do
    ...
    def build(<< partition :: 32, partition_error_code :: 16, base_offset :: 64, log_append_time :: 64-signed, rest :: binary >>) do
      {%PartitionResponse{partition: partition, error_code: error_code(partition_error_code), base_offset: base_offset, log_append_time: log_append_time}, rest}
    end
  end
  ...
  defmodule Response do
    ...
    def parse({:ok, << correlation_id :: 32, responses_length :: 32, rest :: binary>>}) do
      {responses, << throttle_time_ms :: 32 >>} = parse_list(responses_length, rest, &Kafkex.Protocol.Produce.TopicResponse.build/1)
